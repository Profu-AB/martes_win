...
wsl -d %DISTRO_NAME% bash -c "sudo bash -c 'id -u martes &>/dev/null || adduser --disabled-password --gecos \"\" martes; echo \"%LINUX_USER_NAME% ALL=(martes) NOPASSWD: ALL\" > /etc/sudoers.d/%LINUX_USER_NAME%; echo \"martes ALL=(ALL) NOPASSWD: ALL\" > /etc/sudoers.d/martes; rm -rf /home/martes/martes_setup'"
