...
#wsl -d %DISTRO_NAME% --user %LINUX_USER_NAME% bash -lc "set -e; HASH=\$(code-server hash-password < '%PWFILE%'); mkdir -p ~/.config/code-server; printf 'bind-addr: 127.0.0.1:8080\nauth: password\nhashed-password: %s\ncert: false\n' \"$HASH\" > ~/.config/code-server/config.yaml; chmod 600 ~/.config/code-server/config.yaml"


REM 3) Start code-server (background). If you later enable systemd in WSL, you can switch to the service.
REM wsl -d %DISTRO_NAME% -- bash -lc "runuser -l %LINUX_USER_NAME% -c 'nohup code-server >/dev/null 2>&1 &'"

wsl -d %DISTRO_NAME% --user %LINUX_USER_NAME% bash -lc "systemctl --user enable --now code-server"
